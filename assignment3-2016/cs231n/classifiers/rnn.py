...
class CaptioningRNN(object):
  ...
  def loss(self, features, captions):
    ...
    scores, af_cache = temporal_affine_forward(h, W_vocab, b_vocab)
    loss, da = temporal_softmax_loss(scores, captions_out, mask)
    dh, grads['W_vocab'], grads['b_vocab'] = temporal_affine_backward(da, af_cache)
    if self.cell_type == 'rnn':
  ...
  def sample(self, features, max_length=30):
    ...
    H = Wh.shape[0]
    h0 = features.dot(W_proj) + b_proj
    x = W_embed[self._start]*np.ones((N,1))
    next_h = h0
